...
class FewRelDataset(data.Dataset):
    ...
    def __getrel__(self, item):# 对关系描述进行编码
    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            ...
            for j in indices:
                ...
                word = torch.tensor(word).long()#[125]
                pos1 = torch.tensor(pos1).long()#19
                pos2 = torch.tensor(pos2).long()#25
                mask = torch.tensor(mask).long()#[125]
                if count < self.K:
